...
const getBlobHash = async (blob: Blob): Promise<string> => {
  ...
  // Fallback for non-secure origins (e.g. LAN URL over http) where subtle crypto is unavailable.
...
const extractViaPdfJs = async (
  ...
  const isImageLikeObject = (obj: any): boolean => {
    if (!obj) return false;
    const hasSize = Number(obj.width) > 0 && Number(obj.height) > 0;
    const hasPixels = !!obj.bitmap || !!obj.data;
    return hasSize && hasPixels;
  };

  const collectFromObjStore = (objStore: any, collector: Set<any>) => {
    if (!objStore) return;

    // Newer PDF.js internals may use a Map-like _objs structure.
    const rawStore = objStore._objs;
    if (rawStore instanceof Map) {
      for (const value of rawStore.values()) {
        if (isImageLikeObject(value)) collector.add(value);
      }
  ...
  const getObjectFromStore = (
    ...
    return new Promise((resolve) => {
      ...
      try {
        if (typeof objStore?.has === "function" && objStore.has(key)) {
          const direct = objStore.get(key);
          if (direct) {
            done(direct);
            return;
          }
        ...
        // Ignore and fallback to callback style getter.
      }

      try {
        objStore?.get?.(key, (resolved: any) => done(resolved ?? null));
      } catch {
        done(null);
        return;
      }

      // Prevent hanging if object never resolves.
  ...
  for (let pageNum = 1; pageNum <= numPages; pageNum++) {
    ...
    try {
      ...
      // 3. Deep Object Scan via operators + safe store fallback.
      const uniquePageObjects = new Set<any>();
      ...
      for (let i = 0; i < ops.fnArray.length; i++) {
        ...
        if (isImageLikeObject(firstArg)) {
          uniquePageObjects.add(firstArg);
          continue;
        }

        if (typeof firstArg === "string") {
          const fromPageObjs = await getObjectFromStore(
            (page as any).objs,
            firstArg,
          );
          if (isImageLikeObject(fromPageObjs))
            uniquePageObjects.add(fromPageObjs);

          const fromCommonObjs = await getObjectFromStore(
            (page as any).commonObjs,
            firstArg,
          );
          if (isImageLikeObject(fromCommonObjs))
            uniquePageObjects.add(fromCommonObjs);
        }
      }

      // Fallback to store scan for any objects referenced indirectly in forms/patterns.
      collectFromObjStore((page as any).objs, uniquePageObjects);
      collectFromObjStore((page as any).commonObjs, uniquePageObjects);
  ...
  try {
    ...
    for (const [ref, obj] of indirectObjects) {
      if (
        ...
        const height =
          (obj.dict.lookup(PDFName.of("Height")) as any)?.value || 0;
        ...
        if (Array.isArray(filter) && filter.length > 0) {
          ...
          if (!hasFlate) {
            if (filters.some((f) => f === PDFName.of("DCTDecode")))
              isJpeg = true;
            if (filters.some((f) => f === PDFName.of("JPXDecode")))
              isJp2 = true;
          }
        }
...
export const processPdf = async (
  ...
  const pdfJsPromise = extractViaPdfJs(pdfJsDoc, onProgress);
